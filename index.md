...
[`fb_tbl()`](https://docs.ropensci.org/rfishbase/reference/fb_tbl.md)
function, with legacy helper functions for common tables like
[`species()`](https://docs.ropensci.org/rfishbase/reference/species.md)
are still accessible and can still optionally filter by species name
where appropriate. As before, loading the full tables and sub-setting
manually is still recommended.

Historic helper functions like
[`load_taxa()`](https://docs.ropensci.org/rfishbase/reference/load_taxa.md)
(combining the taxonomic classification from Species, Genus, Family and
Order tables),
[`validate_names()`](https://docs.ropensci.org/rfishbase/reference/validate_names.md),
and
[`common_to_sci()`](https://docs.ropensci.org/rfishbase/reference/common_to_sci.md)
and
[`sci_to_common()`](https://docs.ropensci.org/rfishbase/reference/common_names.md)
...
## A message from the FishBase team

> Thank you for using FishBase and SeaLifeBase through the rfishbase
> package, representing more than 35 and 20 years of efforts,
> respectively. With databases of such a large scope, both in terms of
> species and life traits covered, one cannot expect that it is complete
> and free of errors. We would be grateful if you could spare some of
> your time, to directly report to FishBase and SeaLifeBase team:
>
> - mistakes, typos, taxonomic issues, etc.;
> - if you have completed the dataset on your own, by sending the pdfs
>   or reference citations that would allow us to address some missing
>   data;
> - by sending articles that you have published using the data, pdfs or
>   reference citations.
>
> Please send your comments and corrections to <fishbase@q-quatics.org>.
> You may send PDF files directly to the same address.
...
You can see all the tables using
[`fb_tables()`](https://docs.ropensci.org/rfishbase/reference/fb_tables.md)
to see a list of all the table names (specify `sealifebase` if desired).
Careful, there are a lot of them! The fishbase databases have grown a
lot in the decades, and were not intended to be used directly by most
end-users, so you may have considerable work to determine what’s what.
Keep in mind that many variables can be estimated in different ways
(e.g. trophic level), and thus may report different values in different
tables. Also note that species is name (or SpecCode) is not always the
primary key for a table – many tables are specific to stocks or even
individual samples, and some tables are reference lists that are not
species focused at all, but meant to be joined to other tables
(`faoareas`, etc). Compare tables against what you see on fishbase.org,
or ask on our issues forum for advice!
...
[`load_taxa()`](https://docs.ropensci.org/rfishbase/reference/load_taxa.md),
which provides a joined table of taxonomy from subspecies up through
Class, along with the corresponding FishBase taxon ids codes. Here is an
example workflow joining two of the spawning tables and filtering to the
...
`rfishbase` can also provide tables of
[`synonyms()`](https://docs.ropensci.org/rfishbase/reference/synonyms.md),
a table of
[`common_names()`](https://docs.ropensci.org/rfishbase/reference/common_names.md)
in multiple languages, and convert
[`common_to_sci()`](https://docs.ropensci.org/rfishbase/reference/common_to_sci.md)
or
[`sci_to_common()`](https://docs.ropensci.org/rfishbase/reference/common_names.md)
